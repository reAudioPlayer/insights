...
onMounted(() => {
  if (!chart.value || !stereoFieldChart.value || !tonalBalanceChart.value)
    return;
  ...
  setInterval(() => {
    if (!insights.player.playing) return;
...
.left {
  ...
  .loudness-chart {
    height: calc(
      2 * (100vh - var(--h-header) - 4px - var(--h-player) - 2.25rem - 5em) / 3
    );
  }

  .tonal-balance-chart {
    height: calc(
      1 * (100vh - var(--h-header) - 4px - var(--h-player) - 2.25rem - 5em) / 3
    );
  }
}
